            ...
            with torch.no_grad():
                with tqdm(valid_loader, mininterval=5.0, maxinterval=50.0) as it:
                    for batch_no, valid_batch in enumerate(it, start=1):
                        loss = model(valid_batch, adj, is_train=0)
...
def evaluate(model, test_loader, nsample=100, scaler=1, mean_scaler=0, test_scaler=None, foldername="", adj=None):
    with torch.no_grad():
        model.eval()
        mse_total = 0
        mae_total = 0
        ...
        with tqdm(test_loader, mininterval=5.0, maxinterval=50.0) as it:
            for batch_no, test_batch in enumerate(it, start=1):
                output = model.evaluate(test_batch, nsample, adj)
