...
from diff_models2 import diff_CSDI
...
class PriSTI(nn.Module):
    ...
    def calc_loss_valid(
        self, observed_data, cond_mask, observed_mask, side_info, itp_info, is_train, adj
    ):
        loss_sum = 0
        for t in range(self.num_steps):  # calculate loss for all t
            loss = self.calc_loss(
                observed_data, cond_mask, observed_mask, side_info, itp_info, is_train, adj, set_t=t
            )
            loss_sum += loss.detach()
        return loss_sum / self.num_steps

    def calc_loss(
        self, observed_data, cond_mask, observed_mask, side_info, itp_info, is_train, adj, set_t=-1,
    ):
        ...
        predicted = self.diffmodel(total_input, side_info, t, itp_info, cond_mask, adj)
    ...
    def impute(self, observed_data, cond_mask, side_info, n_samples, itp_info, adj):
        B, K, L = observed_data.shape
        ...
        for i in range(n_samples):
            ...
            for t in range(self.num_steps - 1, -1, -1):
                ...
                predicted = self.diffmodel(diff_input, side_info, torch.tensor([t]).to(self.device), itp_info, cond_mask, adj)
    ...
    def forward(self, batch, adj, is_train=1):
        (
            observed_data,
            observed_mask,
            observed_tp,
        ...
        output = loss_func(observed_data, cond_mask, observed_mask, side_info, itp_info, is_train, adj=adj)
        return output

    def evaluate(self, batch, n_samples, adj):
        (
            observed_data,
            observed_mask,
            observed_tp,
        ...
        with torch.no_grad():
            ...
            samples = self.impute(observed_data, cond_mask, side_info, n_samples, itp_info, adj=adj)
