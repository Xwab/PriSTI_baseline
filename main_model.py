...
class PriSTI(nn.Module):
    ...
    def forward(self, batch, is_train=1):
        ...
        output = loss_func(observed_data, gt_mask, observed_mask, side_info, itp_info, is_train)
        return output
    ...
    def evaluate(self, batch, n_samples):
        ...
        with torch.no_grad():
            cond_mask = gt_mask
            target_mask = 1 - cond_mask#observed_mask - cond_mask
...
class PriSTI_ELE(PriSTI):
    def __init__(self, config, device, target_dim=427, seq_len = 16):
        super(PriSTI_ELE, self).__init__(target_dim, seq_len, config, device)
        self.config = config
...
class PriSTI_Metro(PriSTI):
    def __init__(self, config, device, target_dim=81, seq_len = 16):
        super(PriSTI_Metro, self).__init__(target_dim, seq_len, config, device)
        self.config = config
