...
class SpatialConvOrderK(nn.Module):
    ...
    def forward(self, x, support, mask):
        ...
        # mask [batch , nodes, nodes, steps] #在函数内部做重新归一化
        if x.dim() < 4:
            squeeze = True
        ...
        for a in support:
            #a = a.unsqueeze(0).unsqueeze(-1).repeat(B,1,1,L)
            #a_m = a * mask
            #a = a_m / (a_m.sum(2, keepdims=True) + 1e-5) * (a.sum(2, keepdims=True))
            '''a = support[i]
            if i <= 1:
                a = torch.einsum('nwvl, wv->nwvl', (renormal, a)).contiguous()
